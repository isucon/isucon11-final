...
選手自らが設定変更等により競技環境を破壊するなどして、初期状態に戻す必要がある場合はポータルから質問してください。サーバーの再起動あるいは初期化を行います。
再構築以前の競技環境上で変更を加えたソースコードや設定ファイル等の移行が必要な場合は、各チームの責任で行ってください。
...
なお、初期状態ではユーザー以外のデータが存在しないため一度負荷走行を実行してください。
...
TODO: 決める。
`GET /users/me/grades` のレスポンスに含まれる値については、以下の範囲で絶対誤差を許容します。
...
負荷走行において、ベンチマーカーは現実とは関係ない日時を使用します。

### 動作確認用ユーザー

ベンチマーカーは、学生・教員の 1 人としてそれぞれ `isucon`・`isuT` を必ず利用します。
`isucon`・`isuT` ユーザーを用いることで、負荷走行後のアプリケーションの状態確認が可能です。
