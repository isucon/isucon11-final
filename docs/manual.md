...
- **その他、主催者による追試を妨げる変更（例： サーバー上の `isucon` 以外のユーザー関する、ユーザー除や既存の公開鍵の削除）を行ってはいけません。**
...
なお、ベンチマーカー内のユーザー独立しているため、 `Cache-Control: public` 等が指定されていたとしても、ユーザー士でキャッシュを共有することはありません。
...
- 競技中のスコアが、最初に 80,000 点を超えた 1 チームを特別賞とします。
...
1. 全チームのサーバーを再起動し、10 分以上待ちます。
    ...
    - すでにシャットダウンされているなどの理由により、`sudo systemctl poweroff`を実行できなかったサーバー存在する場合は失格となります。
...
各サーバーおけるソフトウェアの入れ替え、設定の変更、アプリケーションコードの変更および入れ替えなどは一切禁止しない。
