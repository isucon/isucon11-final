...
主催者側の責によって特定のチームの競技進行に不利益が認められる時、主催者はそのチームに対して競技時間の補填を行うことがあります。この際、補填時間の算出は不具合等を主催者が認識してからそれが解決されるまでにかかった時間までを基準とします。つまり、選手は疑問点や不具合の報告を速やかに質問にて行うことで、競技進行に支障があった時間を客観的に明らかにすることができるため、主催者はこれを推奨します。

なお、全チームで確実かつ同様に発生する不具合 (ベンチマーカーの不具合など) は特定チームにおける競技進行の妨げとはならないため、補填の対象とは通常考えられません (この限りではありません)。補填の対象になる例としては、主催者側のミスで特定チームのみサーバーへ接続できない状態 (設定不備) などが挙げられます。
