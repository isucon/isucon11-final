...
最終スコアが確定後、主催者による確認作業（追試）を行います。
下記の点が確認できなかった場合は fail となります。
...
1. 全チームのサーバーを再起動し、10 分以上待ちます。
   ...
   - **お願い**: 再起動のテストのために、選手によって意図的に `poweroff` されたサーバーの起動依頼については運営で対応する余力がないため、ベストエフォートでの対応になります。確実にシステムを再起動するため、再起動手順をこのように定義していますが、競技時間中に再起動をテストする場合は `sudo systemctl reboot` 等で実施してください
