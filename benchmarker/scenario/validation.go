...
func validateUserGrade(expected *model.GradeRes, actual *api.GetGradeResponse) error {
	if !AssertEqual("CourseResults length", len(expected.CourseResults), len(actual.CourseResults)) {
	...
	for _, courseResult := range actual.CourseResults {
		if _, ok := expected.CourseResults[courseResult.Code]; !ok {
			ContestantLogger.Printf("extra course: %v", courseResult.Code)
...
func validateSummary(expected *model.Summary, actual *api.Summary) error {
	if !AssertEqual("credits", expected.Credits, actual.Credits) {
		ContestantLogger.Println("credits. expected: ", expected.Credits, "actual: ", actual.Credits)
		return failure.NewError(fails.ErrCritical, errInvalidResponse("成績確認のsummaryのcreditsが一致しません"))
	}

	if !AssertAbsolute("summary gpa", expected.GPA, actual.GPA, validateGPAErrorTolerance) {
		return failure.NewError(fails.ErrCritical, errInvalidResponse("成績確認のsummaryのgpaが一致しません"))
	}

	if !AssertAbsolute("summary gpa avg.", expected.GpaAvg, actual.GpaAvg, validateGPAErrorTolerance) {
		return failure.NewError(fails.ErrCritical, errInvalidResponse("成績確認のsummaryのgpa_avgが一致しません"))
	}

	if !AssertAbsolute("summary gpa max", expected.GpaMax, actual.GpaMax, validateGPAErrorTolerance) {
		return failure.NewError(fails.ErrCritical, errInvalidResponse("成績確認のsummaryのgpa_maxが一致しません"))
	}

	if !AssertAbsolute("summary gpa min", expected.GpaMin, actual.GpaMin, validateGPAErrorTolerance) {
		ContestantLogger.Println("gpamin. expected: ", expected.GpaMin, "actual: ", actual.GpaMin)
		return failure.NewError(fails.ErrCritical, errInvalidResponse("成績確認のsummaryのgpa_minが一致しません"))
	}

	if !AssertAbsolute("summary gpaTscore", expected.GpaTScore, actual.GpaTScore, validateGPAErrorTolerance) {
		ContestantLogger.Println("gpatscore. expected: ", expected.GpaTScore, "actual: ", actual.GpaTScore)
...
func validateCourseResult(expected *model.CourseResult, actual *api.CourseResult) error {
	if !AssertEqual("course name", expected.Name, actual.Name) {
		ContestantLogger.Println("name. expected: ", expected.Name, "actual: ", actual.Name)
	...
	if !AssertEqual("TotalScore", expected.TotalScore, actual.TotalScore) {
		ContestantLogger.Println("TotalScore. expected: ", expected.TotalScore, "actual: ", actual.TotalScore)
		return failure.NewError(fails.ErrCritical, errInvalidResponse("成績確認のコースのTotalScoreが一致しません"))
	}

	if !AssertEqual("TotalScoreMax", expected.TotalScoreMax, actual.TotalScoreMax) {
		ContestantLogger.Println("TotalScoreMax. expected: ", expected.TotalScoreMax, "actual: ", actual.TotalScoreMax)
		return failure.NewError(fails.ErrCritical, errInvalidResponse("成績確認のコースのTotalScoreMaxが一致しません"))
	}

	if !AssertEqual("TotalScoreMin", expected.TotalScoreMin, actual.TotalScoreMin) {
		return failure.NewError(fails.ErrCritical, errInvalidResponse("成績確認のコースのTotalScoreMinが一致しません"))
	}

	if !AssertAbsolute("TotalScoreAvg", expected.TotalScoreAvg, actual.TotalScoreAvg, validateTotalScoreErrorTolerance) {
		return failure.NewError(fails.ErrCritical, errInvalidResponse("成績確認のコースのTotalScoreAvgが一致しません"))
	}

	if !AssertAbsolute("TotalScoreTScore", expected.TotalScoreTScore, actual.TotalScoreTScore, validateTotalScoreErrorTolerance) {
		return failure.NewError(fails.ErrCritical, errInvalidResponse("成績確認のコースのTotalScoreTScoreが一致しません"))
	}

	if !AssertEqual("ClassScores length", expected.ClassScores, actual.ClassScores) {
...
func validateClassScore(expected *model.ClassScore, actual *api.ClassScore) error {
	if !AssertEqual("class ID", expected.ClassID, actual.ClassID) {
		ContestantLogger.Println("classID. expected: ", expected.ClassID, "actual: ", actual.ClassID)
	...
	if !AssertEqual("class part", expected.Part, actual.Part) {
		ContestantLogger.Println("part. expected: ", expected.Part, "actual: ", actual.Part)
	...
	if !AssertEqual("class title", expected.Title, actual.Title) {
		ContestantLogger.Println("title. expected: ", expected.Title, "actual: ", actual.Title)
		return failure.NewError(fails.ErrCritical, errInvalidResponse("成績確認のクラスのタイトルが一致しません"))
	}

	if !AssertEqual("class score", expected.Score, actual.Score) {
		ContestantLogger.Println("score. expected: ", expected.Score, "actual: ", actual.Score)
		return failure.NewError(fails.ErrCritical, errInvalidResponse("成績確認のクラスのスコアが一致しません"))
	}

	if !AssertEqual("submitters count", expected.SubmitterCount, actual.Submitters) {
		ContestantLogger.Println("submitters. expected: ", expected.SubmitterCount, "actual: ", actual.Submitters)
